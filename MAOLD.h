...
void getRestaurant(int restIndex, Restuarant_Dist* rest_distance) { 
  /*
...
void nearest21_restaurants(Restuarant_Dist* rest_distance){
  ...
  if(buttonVal == LOW){
    //int16_t x = lon_to_x - yegMapX;
    //int16_t y = lon_to_y - yegMapY;
    //int16_t manhattan_dist = abs(x) + abs(y);
    for(int16_t i = 0 ; i <(NUM_RESTAURANTS-1) ;i++){
      getRestaurant(i,&rest_distance);
      rest_distance[i].index = i;
      rest_distance[i].dist = (abs(lon_to_x - yegMapX)) + (abs(lon_to_y - yegMapY));
    }
